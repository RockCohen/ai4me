...
export const SamplerSim = (function () {
  ...
  function dist(contextChar, temperature, topK) {
    ...
    const m = Math.max(...masked.filter(x => x > -Infinity));
    const es = masked.map(l => l === -Infinity ? 0 : Math.exp(l - m));
  ...
  function mount(container) {
    ...
    function renderBars() {
      ...
      ctx.fillText(`给定上文「${ctxChar}」→ 下一个字符的分布（T=${t.toFixed(1)}, top-k=${k}）`, 14, 24);
      // 取概率最大的前 14 个画柱状图
      const order = p.map((v, i) => [v, i]).sort((a, b) => b[0] - a[0]).slice(0, 14);
    ...
    $('.sp-one').onclick = () => { const { t, k } = params(); append(chars[sample(dist(ctxChar, t, k))]); renderBars(); };
    $('.sp-gen').onclick = () => {
      const { t, k } = params();
      let acc = '';
      for (let i = 0; i < 120; i++) { const ch = chars[sample(dist(ctxChar, t, k))]; acc += ch; ctxChar = ch; }
      append(acc);
      renderBars();
    };
