...
<h3>一、场景切入（数字算给你看）</h3>
<p>你问框架一个问题：x = 3 时，L = (2x)² 对 x 的导数是多少？</p>
...
<p><b>形式化</b>：若 L = f(g(x))，则 dL/dx = f′(g(x)) · g′(x)。两站如此，一百站也如此——连乘而已。</p>
...
<h3>四、数学展开（逐符号）</h3>
<p>符号逐个定义：<b>dL/da</b> 读作"a 挪 1 小格时 L 挪多少格"——它是<b>敏感度</b>，不是"两个 d 相除"（虽然巧合的是：把 d 当分数约掉，链式法则的形式恰好正确，这也是它好记的原因）。</p>
<p>为什么局部斜率是<b>乘</b>进链条的？在 a = 6 附近做一阶展开：L ≈ L(6) + 12·(a − 6)。a 挪 1 格，L 挪 12 格——12 就是"兑换汇率"。上游传来"b 挪 1 格相当于 L 挪 12 格"，而 a 挪 1 格相当于 b 挪 2 格，于是 a 挪 1 格 = L 挪 24 格。汇率连乘，就是链式法则。</p>
<p>一般两站：L = f(g(x)) → dL/dx = f′(g(x)) · g′(x)。一百站：一百个局部斜率连乘。变量有一万个？反向一遍，一万个梯度同时到手——这就是反向模式微分的经济学（对比：前向模式每问一个输入要遍一次图）。</p>
...
<li>二阶导数 d²L/dx² 能用同样的图算吗？（能——对图再反向一次，但成本翻倍；深度学习几乎不用）</li>
</ol>
...
<p>9 个数据点：(0.1, 1.21)、(0.2, 1.29)、(0.3, 1.35)…大致贴在 y = 0.9x + 0.15 附近（带一点噪声）。模型：ŷ = w·x + b。目标：让线自动贴上去。</p>
...
<p>损失定义：<b>L(w, b) = (1/n)·Σᵢ₌₁ⁿ (ŷᵢ − yᵢ)²</b>，其中 ŷᵢ = w·xᵢ + b，n = 9。</p>
<p>逐符号：ŷᵢ 读"w·x 加 b 在第 i 个点的预测"；Σᵢ₌₁ⁿ 表示"把 i 从 1 加到 n"；平方惩罚"偏得越远罚越重"。</p>
<p>对 w 求偏导（链式法则两站：外层平方、内层乘 x）：</p>
<p style="text-align:center"><b>∂L/∂w = (2/n)·Σᵢ (ŷᵢ − yᵢ)·xᵢ</b></p>
<p>逐项拆解：外层 (ŷᵢ−yᵢ)² 的导数 = 2(ŷᵢ−yᵢ)（"平方站"）；内层 ŷᵢ 对 w 的导数 = xᵢ（"乘法站"，局部斜率 = 对方因子）。<b>这正是 c03 说的两站连乘</b>——你在 c07 手写的 ×2 站和乘法站，在这里合体了。</p>
<p>对 b 同理但内层导数是 1：∂L/∂b = (2/n)·Σ(ŷᵢ−yᵢ)。</p>
<h3>三、直觉 → 转译 → 形式化</h3>
<p><b>直觉</b>：站在山坡上，看不见谷底，但看得见脚下的坡度——朝坡下走一小步，永远没错。</p>
<p><b>转译</b>："坡度" = 梯度 ∇L；"一小步" = 学习率 η；"朝坡下" = 取负号。合起来：θ ← θ − η·∇L(θ)。</p>
<p><b>形式化</b>：对线性回归，其实存在一步到位的解析解（正规方程 w = (XᵀX)⁻¹Xᵀy）。那为什么还学梯度下降？——正规方程只适用线性模型 + 要矩阵求逆；而梯度下降<b>对任何可导损失都通用</b>。学会走山路，才能去没有缆车的山。</p>
<h3>四、发散问题</h3>
<ol>
<li>lr 多大时会发散？临界值和数据的什么性质同尺度？（提示：与 Σx² 相关——这正是"特征缩放"存在的理由）</li>
<li>为什么用平方损失而不是绝对值损失？（可导、凸、高斯噪声下的最大似然——三个理由各管一层）</li>
<li>如果把 9 个点的 x 全部乘 100，同样的 lr 会发生什么？（→ 特征缩放问题的入口）</li>
<li>把 x 换成 (1, x, x²) 三列，同一个梯度下降还能拟合抛物线吗？（能——"线性"指的是对参数线性；这也是多项式回归与特征工程的入口）</li>
</ol>
...
<p>完整公式：<b>Attention(Q, K, V) = softmax(QKᵀ/√dₖ)·V</b>。逐符号：Q 是所有 token 的查询堆成的矩阵（T×dₖ），K 是标签矩阵，V 是内容矩阵，dₖ 是每头维度。</p>
<p><b>因果掩码</b>在打分之后、softmax 之前：把 j &gt; i 的位置置 −∞ → exp 后为 0 → token i 看不见未来。训练时"预测下一个词"不许偷看答案，全靠这一步。</p>
<p><b>√d 的方差推导</b>（论文一句话背后的完整账）：设 q、k 的每个分量独立、均值 0、方差 1，则点积 q·k = Σqᵢkᵢ 的方差 = d（独立项方差相加）。d=64 时打分的标准差 ≈ 8——打分尺度随维度膨胀，softmax 被推进饱和区（一个候选独占 100%，其余归零），梯度消失。除以 √d 把方差拉回 1，可训练性保住了。</p>
<h3>三、直觉 → 转译 → 形式化</h3>
