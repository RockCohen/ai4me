...
<li><b>延伸 · 同一哲学的生态复制</b>：pandas 的视图式切片、数据库的物化视图、Git 的轻量分支——"改说明书，不搬数据"是整个软件世界的第一性原理之一。学的是张量，懂的是设计。</li>
  ...
  // ============ 组 B · autograd ============
  {
    ...
    read: String.raw`
<p class="tip">🔗 承接栏：本章坐实两个缺口——c02 末尾"改视图为什么牵动原张量"（数据的引用链），以及那句"调 backward() 梯度就有了"（计算的引用链）。它同时是 c05（线性回归的梯度从哪来）、c07（全图怎么反向）、c08（MLP 怎么训）的地基。阶段 0 开始以来欠你的这句解释，本章还清。</p>
<h3>〇、四问</h3>
<ul>
<li><b>目的</b>：不手推公式，让任何复合函数的导数"自动可得"——这是深度学习框架存在的第一理由；</li>
<li><b>形式</b>：动态构建有向无环图（计算图）+ 一次反向遍历——学名叫<b>反向模式自动微分</b>；</li>
<li><b>质料</b>：张量的三样东西——<code>.requires_grad</code>（要不要记账）、<code>.grad_fn</code>（账本条目）、<code>.grad</code>（回传结果）；</li>
<li><b>动力</b>：训练循环五件套的第四件（backward）。没有它，c05 的线性回归只能手推公式，c08 的 MLP 干脆训不了。</li>
</ul>
<h3>一、场景切入（数字算给你看）</h3>
<p>你问框架一个问题：$x = 3$ 时，$L = (2x)^2$ 对 $x$ 的导数是多少？</p>
<p>手算你都会：L = 36，dL/dx = 2·(2x)·2 = 24。但<b>你一行导数代码都没写</b>——只写了前向。框架凭什么知道导数？它不"懂"微积分：它在你每次做运算时<b>记了一笔账</b>——谁从谁算出来的、这一站的局部斜率是多少。这本账，就是计算图。</p>
<h3>二、直觉 → 转译 → 形式化</h3>
<p><b>直觉</b>：多米诺骨牌倒放。每张牌倒下时，把"我倒的力度"乘上"我和前一张牌的联动系数"，传给前一张。</p>
<p><b>转译</b>："力度"= 上游传来的梯度；"联动系数" = 这一站的局部导数；"传给前一张" = 逆着图走。<b>链式法则</b>说的就是：整条链的敏感度 = 沿途局部敏感度的连乘。</p>
<p><b>形式化</b>：若 $L = f(g(x))$，则 $\dfrac{dL}{dx} = f'(g(x)) \cdot g'(x)$。两站如此，一百站也如此——连乘而已。</p>
<h3>三、贯穿示例（一本账走完全程）</h3>
<p>就用 L = (2x)²，x = 3。<b>前向</b>：数字从左流向右，同时每站记下自己的局部斜率。</p>
<pre>x = 3  ──×2──▶  a = 6  ──□²──▶  L = 36
账本：
  站 a ：值 6，局部斜率 = 2        （∂a/∂x = 2）
  站 L ：值 36，局部斜率 = 2a = 12 （∂L/∂a = 2a）</pre>
<p><b>反向</b>：种子 dL/dL = 1 从右往左，每站把"传来的梯度 × 自己的局部斜率"写给上游。</p>
<pre>dL/dL = 1
dL/da = 1 × 12 = 12
dL/dx = 12 × 2 = 24   ✓ 与手算 2·(2x)·2 = 24 完全一致</pre>
<p>右侧交互台就是这本账的可视化：每站口袋里的 data / 局部斜率 / 累计梯度，反向时逐站点亮。</p>
<h3>四、数学展开（逐符号）</h3>
<p>符号逐个定义：<b>$\frac{dL}{da}$</b> 读作"$a$ 挪 1 小格时 $L$ 挪多少格"——它是<b>敏感度</b>，不是"两个 d 相除"（虽然巧合的是：把 $d$ 当分数约掉，链式法则的形式恰好正确，这也是它好记的原因）。</p>
<p>为什么局部斜率是<b>乘</b>进链条的？在 $a = 6$ 附近做一阶展开：$L \approx L(6) + 12\,(a - 6)$。$a$ 挪 1 格，$L$ 挪 12 格——12 就是"兑换汇率"。上游传来"$b$ 挪 1 格相当于 $L$ 挪 12 格"，而 $a$ 挪 1 格相当于 $b$ 挪 2 格，于是 $a$ 挪 1 格 = $L$ 挪 24 格。汇率连乘，就是链式法则。</p>
<p>一般两站：$L = f(g(x)) \Rightarrow \frac{dL}{dx} = f'(g(x)) \cdot g'(x)$。一百站：一百个局部斜率连乘。变量有一万个？反向一遍，一万个梯度同时到手——这就是反向模式微分的经济学（对比：前向模式每问一个输入要遍一次图）。</p>
<h3>五、发散问题（导向缺口与下一站）</h3>
<ol>
<li>两个变量 x、y 都影响 L，反向要遍几次图？（一次——这就是反向模式赢的原因，c05 马上用到）</li>
<li>PyTorch 为什么默认<b>不清零</b> .grad？累加语义的合法用例是什么？（→ c04）</li>
<li>如果反向到一半某站忘了回传，会发生什么？报错吗？（→ c07 的静默错误）</li>
<li>二阶导数 $\frac{d^2 L}{dx^2}$ 能用同样的图算吗？（能——对图再反向一次，但成本翻倍；深度学习几乎不用）</li>
</ol>
<p class="soul">🤔 留给你想：链式法则高中就学过，为什么直到 1986 年才有人拿它训练网络？（提示：缺的不是数学，是"怎么组织计算"——这本书后面你会反复见到：伟大的应用 = 老数学 × 新组织。）</p>
<details class="refs"><summary>🏛 权威佐证与延伸</summary><ul>
...
<li><b>延伸 · "显式"的边界</b>：not 所有框架都这么倔——Keras 把清零藏进 fit() 里，好用但黑盒。两种设计哲学没有对错，只有"你在学什么"：Keras 教你用工具，PyTorch 教你工具的原理。本阶段选后者，是有意的。</li>
  ...
  },
  {
    ...
    read: String.raw`
<p class="tip">🔗 承接栏：本章坐实 c03 的"自动求导能干什么"——答案是一个 218 年前的问题。1801 年高斯用最小二乘从零散观测算出了失踪谷神星的轨道；1886 年 Galton 研究父母与子女身高时给这方法起了名（regression，"向平庸回归"）。本章把训练循环五件套第一次完整合体——它是 c08 MLP 训练的唯一模板。</p>
<h3>〇、四问</h3>
<ul>
<li><b>目的</b>：不用 nn.Linear、不用优化器，15 行写完"训练"这个概念的最小完备样本；</li>
<li><b>形式</b>：MSE 损失 + 梯度下降——高斯当年用解析解，我们走数值路线（它通用于一切可导损失，这正是进步所在）；</li>
<li><b>质料</b>：两个可训练标量 w、b——小到能徒手验算每一个梯度；</li>
<li><b>动力</b>：c08 的 MLP 训练循环与此<b>一字不差</b>，只是 forward 换成了 89 个参数的网络。</li>
</ul>
<h3>一、场景与贯穿示例（一组真实数字走完全程）</h3>
<p>9 个数据点：(0.1, 1.21)、(0.2, 1.29)、(0.3, 1.35)…大致贴在 $y = 0.9x + 0.15$ 附近（带一点噪声）。模型：$\hat{y} = w\cdot x + b$。目标：让线自动贴上去。</p>
<p><b>初始 w = −2，b = 4</b>——歪得离谱。把第一步的所有数字算给你看：</p>
<pre>前向（以 x = 0.5 这个点为例）：
  ŷ = w·x + b = −2×0.5 + 4 = 3.0
  真实 y = 0.6 → 误差 (ŷ−y) = 2.4

全部 9 个点算完：
  L = MSE = (1/9)·Σ(ŷᵢ − yᵢ)² = 9.87

反向（梯度公式见下节推导）：
  ∂L/∂w = (2/9)·Σ(ŷᵢ−yᵢ)·xᵢ = −4.31
  ∂L/∂b = (2/9)·Σ(ŷᵢ−yᵢ)    = −5.12

更新（lr = 0.6）：
  w ← −2 − 0.6×(−4.31) = 0.59   （朝 0.9 挪）
  b ← 4 − 0.6×(−5.12)  = 7.07   （过了头，但方向对了）</pre>
<p>重复 200 步：w → 0.9、b → 0.15 附近，loss 一路下滑。右侧模拟器实时演示这条轨迹——点「训练」或「单步」。</p>
<h3>二、数学展开（逐符号，不看框架自己推）</h3>
<p>损失定义：</p>
<p style="text-align:center">$$L(w, b) = \frac{1}{n}\sum_{i=1}^{n}\left(\hat{y}_i - y_i\right)^2, \qquad \hat{y}_i = w\,x_i + b,\quad n = 9$$</p>
<p>逐符号：$\hat{y}_i$ 读"模型在第 $i$ 个点的预测"；$\sum_{i=1}^{n}$ 表示"把 $i$ 从 1 加到 $n$"；平方惩罚"偏得越远罚越重"。</p>
<p>对 $w$ 求偏导（链式法则两站：外层平方、内层乘 $x$）：</p>
<p style="text-align:center">$$\frac{\partial L}{\partial w} = \frac{2}{n}\sum_{i=1}^{n}\left(\hat{y}_i - y_i\right)x_i \qquad \frac{\partial L}{\partial b} = \frac{2}{n}\sum_{i=1}^{n}\left(\hat{y}_i - y_i\right)$$</p>
<p>逐项拆解：外层 $(\hat{y}_i-y_i)^2$ 的导数 = $2(\hat{y}_i-y_i)$（"平方站"）；内层 $\hat{y}_i$ 对 $w$ 的导数 = $x_i$（"乘法站"，局部斜率 = 对方因子）。<b>这正是 c03 说的两站连乘</b>——你在 c07 手写的 ×2 站和乘法站，在这里合体了。</p>
<p>对 $b$ 同理但内层导数是 1：$\frac{\partial L}{\partial b} = \frac{2}{n}\cdot\sum(\hat{y}_i-y_i)$。</p>
<h3>三、直觉 → 转译 → 形式化</h3>
<p><b>直觉</b>：站在山坡上，看不见谷底，但看得见脚下的坡度——朝坡下走一小步，永远没错。</p>
<p><b>转译</b>："坡度" = 梯度 $\nabla L$；"一小步" = 学习率 $\eta$；"朝坡下" = 取负号。合起来：$\theta \leftarrow \theta - \eta\,\nabla L(\theta)$。</p>
<p><b>形式化</b>：对线性回归，其实存在一步到位的解析解（正规方程 $w = (X^\top X)^{-1}X^\top y$）。那为什么还学梯度下降？——正规方程只适用线性模型 + 要矩阵求逆；而梯度下降<b>对任何可导损失都通用</b>。学会走山路，才能去没有缆车的山。</p>
<h3>四、发散问题</h3>
<ol>
<li>lr 多大时会发散？临界值和数据的什么性质同尺度？（提示：与 Σx² 相关——这正是"特征缩放"存在的理由）</li>
<li>为什么用平方损失而不是绝对值损失？（可导、凸、高斯噪声下的最大似然——三个理由各管一层）</li>
<li>把 9 个点的 $x$ 全部乘 100，同样的 lr 会发生什么？（→ 特征缩放问题的入口）</li>
<li>把 $x$ 换成 $(1,\ x,\ x^2)$ 三列，同一个梯度下降还能拟合抛物线吗？（能——"线性"指的是对参数线性；这也是多项式回归与特征工程的入口）</li>
</ol>
<p class="soul">🤔 留给你想：高斯的时代没有梯度、没有算力，他用"令导数为零解方程"找到了答案；我们故意不用解析解，改走 200 步下坡。白费劲吗？——想清楚"哪一步是线性专属、哪一步是通用能力"，你就明白了这门课为什么从这里开始。</p>
<details class="refs"><summary>🏛 权威佐证与延伸</summary><ul>
...
<li><b>李沐论文精读清单</b>（<a href="https://github.com/mli/paper-reading" target="_blank">github.com/mli/paper-reading</a>）。从本阶段毕业后的"论文地图"：每篇都有中文视频笔记，按图索骥即可。</li>
  ...
  },
  {
    ...
    read: String.raw`
<p class="tip">🔗 承接栏：本章坐实 c03 留下的两个缺口——"逆拓扑序"到底是什么序、多条路径的梯度怎么汇合（提示：就是 c04 那个 +=）。贯穿示例仍是交互台的原例 f = a·b + a·c + b·c，这次把每一站的数字全部算完。</p>
<h3>〇、四问</h3>
<ul>
<li><b>目的</b>：单节点的 _backward 有了（c06），全图反向只剩"按什么顺序调用"这一个问题；</li>
<li><b>形式</b>：拓扑排序（1962 年的老算法，Knuth《计算机程序设计艺术》第一卷有经典论述，原本的用场是装配线和菜谱：先切菜再下锅）+ 逆序调用；</li>
<li><b>质料</b>：一个 visited 集合、一个 topo 数组、约 8 行代码；</li>
<li><b>动力</b>：没有它，c06 的单站账本无法升级成 c08 的整网训练。</li>
</ul>
<h3>一、场景与贯穿示例（每站数字全算完）</h3>
<p>规则一句话：<b>一个节点的 _backward，必须在它所有下游消费者都处理完之后才执行</b>——否则它读到的 out.grad 还不完整。拿交互台的原例把整本账算完（a=2, b=−1, c=0.5）：</p>
<pre>【前向】
  p₁ = a·b = −2    p₂ = a·c = 1    p₃ = b·c = −0.5
  s = p₁ + p₂ = −1    L = s + p₃ = −1.5

【反向】种子 dL/dL = 1，逆拓扑遍历：
  加法站 s ：dL/dp₁ = 1　dL/dp₂ = 1     （加法：原样分发）
  加法站 p₃：dL/dp₃ = 1
  乘法站 p₁：∂p₁/∂a = b = −1　∂p₁/∂b = a = 2
  乘法站 p₂：∂p₂/∂a = c = 0.5　∂p₂/∂c = a = 2
  乘法站 p₃：∂p₃/∂b = c = 0.5　∂p₃/∂c = b = −1
...
用 torch 验证：autograd 算出 (−0.5, 2.5, 1.0) ✓ 分毫不差</pre>
<p>注意 dL/da 的两份贡献：<b>必须相加</b>。这就是 c06 问你的"为什么写 += 而不是 ="——菱形依赖（a 同时流进 p₁ 和 p₂）是它的出生现场。</p>
<h3>二、数学展开（为什么必须是逆拓扑）</h3>
<p>乘法节点的回传规则：<b>dL/d输入 = dL/d输出 × 对方因子的值</b>。注意它要用到 <code>out.grad</code>——而 out.grad 只在 out 的<b>所有消费者</b>都完成回传之后才完整。</p>
<p>所以正确顺序是一个<b>依赖调度问题</b>：消费者在后、被消费者在前。拓扑排序给出正序（叶子在前、L 在后），倒过来就是反向传播的执行序。90 行的 micrograd 和上亿参数的 PyTorch，在这件事上没有任何区别。</p>
<p>顺序错了会怎样？<b>不报错</b>——上游节点用残缺的 out.grad 回传，梯度悄悄错掉。这是"静默错误"的教科书案例（c10 的门诊逻辑在这里已经预演过一次）。</p>
<h3>三、发散问题</h3>
<ol>
<li>为什么计算图必须是<b>有向无环</b>的？如果 a 的值反过来依赖 L，会出什么结构问题？（环 = 拓扑排序不存在；循环计算需要另一套工具——这正是"while 循环"在自动微分里特殊的原因）</li>
<li>你的 c07 重写版怎么<b>验证</b>梯度对不对？（→ tutor/gradcheck.py：数值差分当第三方裁判——两个"都可能错"的实现无法互相仲裁）</li>
<li>梯度一次回传的前提是无环。while 循环、递归这类"真循环"怎么办？（→ 展开成有限步，或 SCAN 类算子——自动微分研究的前沿之一）</li>
</ol>
<p class="soul">🤔 留给你想：反向传播 = 链式法则（数学）+ 拓扑排序（调度）。为什么"伟大的算法"经常 = 一个老数学 × 一个朴素调度？你还能想到别的例子吗？（FFT、动态规划。）</p>
<details class="refs"><summary>🏛 权威佐证与延伸</summary><ul>
...
<li><b>延伸 · token 是钱</b>：API 按 token 计费、上下文窗口按 token 计长（GPT-4 约 8K–1M）。学完 BPE 你就能自己估算"一段话值多少钱"。</li>
  ...
  },
  {
    ...
    read: String.raw`
<p class="tip">🔗 承接栏：本章坐实阶段 1 的核心机制缺口——"token 之间怎么交流"。它向上承接 c13（token 化之后的下一步），向下是 c15（多头）、c17（GPT 全景）的地基。交互台里那句 softmax(QKᵀ/√d)·V，本章把每个符号算成具体数字。</p>
<h3>〇、四问</h3>
<ul>
<li><b>目的</b>：翻译到"坐"这个词时该看原文哪个词？整句压成一个向量装不下（Bahdanau 2014 的痛点）——答案：不压缩，按相关度回看全部；</li>
<li><b>形式</b>：每个 token 发三份向量——Q（我在找什么）、K（我是什么标签）、V（我的内容）；打分 → softmax → 加权混合 V；</li>
<li><b>质料</b>：一张 T×T 的权重表（每个 token 对每个 token 的关注度）；</li>
<li><b>动力</b>：没有它，"_THE_ cat" 和 "A cat" 里的 cat 无法按上下文区分——词义随上下文变化的现实就没人处理。</li>
</ul>
<h3>一、贯穿示例（打分 → softmax → 混合，数字全算完）</h3>
<p>句子：[小猫, 坐, 在, 垫子, 上]。以「坐」为查询，它与各 token 打分（已除以 √d，玩具数字）：</p>
<pre>打分：[0.0, 2.0, 1.0, 0.5]

softmax：e⁰=1.00, e²⁰⁰⁄…（各自 exp 后）= [1.00, 7.39, 2.72, 1.65]，总和 12.76
权重  = [7.8%, 57.9%, 21.3%, 12.9%]　← 和恰好 100%（softmax 的定义保证）

输出：坐的新表示 = 7.8%·V(小猫) + 57.9%·V(坐) + 21.3%·V(在) + 12.9%·V(垫子)</pre>
<p>解读：「坐」的新表示里，一大半是它自己，但混进了「垫子」近 13% 的信息——动作和地点的关系被写进了向量。这就是"上下文感知表示"的字面含义。</p>
<h3>二、数学展开（逐符号 + 为什么除以 √d）</h3>
<p>完整公式：</p>
<p style="text-align:center">$$\text{Attention}(Q, K, V) = \mathrm{softmax}\!\left(\frac{QK^\top}{\sqrt{d_k}}\right)V$$</p>
<p>逐符号：$Q$ 是所有 token 的查询堆成的矩阵（$T \times d_k$），$K$ 是标签矩阵，$V$ 是内容矩阵，$d_k$ 是每头维度。</p>
<p><b>因果掩码</b>在打分之后、softmax 之前：把 $j > i$ 的位置置 $-\infty$ → $\exp$ 后为 0 → token $i$ 看不见未来。训练时"预测下一个词"不许偷看答案，全靠这一步。</p>
<p><b>$\sqrt{d_k}$ 的方差推导</b>（论文一句话背后的完整账）：设 $q$、$k$ 的每个分量独立、均值 0、方差 1，则点积 $q\cdot k = \sum_i q_i k_i$ 的方差 $= d_k$（独立项方差相加）。$d_k = 64$ 时打分的标准差 $\approx 8$——打分尺度随维度膨胀，softmax 被推进饱和区（一个候选独占 100%，其余归零），梯度消失。除以 $\sqrt{d_k}$ 把方差拉回 1，可训练性保住了。</p>
<h3>三、直觉 → 转译 → 形式化</h3>
<p><b>直觉</b>：图书馆找书——Q 是你的检索词，K 是每本书的标签卡，V 是书的内容。检索词与标签卡比对（打分），按匹配程度决定"借阅多少内容"（加权混合）。</p>
<p><b>转译</b>：比对 = 点积；借阅比例 = softmax 归一；√d = 防止检索词太长时"什么都像"。</p>
<p><b>形式化</b>：上面那行公式。一个 exel 表格就能手算——右侧交互台算的就是它，真实矩阵运算。</p>
<h3>四、发散问题</h3>
<ol>
<li>Q/K/V 为什么是三份不同投影，而不是同一个向量自己跟自己算？（Q=K 时权重永远最关注自己——"找什么"与"我是什么"必须分开）</li>
<li>训练与推理时，因果掩码的行为有何差异？（无差异——训练时同样禁看未来；差异在 Dropout/BatchNorm，那是 c10 的话题）</li>
<li>为什么叫"注意力"？它和人眼的注意力像在哪、不像在哪？（像：选择性聚焦；不像：没有"看不懂就再看一遍"的反馈回路）</li>
<li>c19 的 config 里有 num_key_value_heads &lt; heads——GQA 怎么把 KV cache 再省几倍？（多个 Q 头共享一组 KV 头）</li>
</ol>
<p class="soul">🤔 留给你想：Q/K/V 为什么要三份不同的向量，而不是用同一个向量"自己跟自己算相似度"？如果 Q=K=V，权重会怎样？（永远最关注自己。）"找什么"和"我是什么"分开，关系才立得起来。</p>
<details class="refs"><summary>🏛 权威佐证与延伸</summary><ul>
